...
class AStarPlayer(Player):
    ...
    def heuristic(a: Position, b: Position) -> int:
        # Manhattan distance (admissible for 4-dir grid)
    ...
    def search_path(self, snake: Snake, food: Food, *obstacles: Set[Obstacle]):
        ...
        # Snake body cannot be entered, except the head (start)
        blocked = set(snake.positions)
        blocked.discard(start)

        # Obstacles: expensive but allowed
        obstacle_set: Set[Obstacle] = obstacles[0] if obstacles else set()
        obstacle_positions = {ob.position for ob in obstacle_set}
        OBSTACLE_PENALTY = 5

        # A* frontier: (priority = g + h, counter, Position)
        frontier = []
        counter = 0
        start_h = self.heuristic(start, goal)
        heapq.heappush(frontier, (start_h, counter, start))

        # g-costs
        cost_so_far = {start: 0}
        # For reconstructing path: node -> (previous_node, direction_from_prev_to_node)
        ...
        while frontier:
            ...
            # Mark expanded node as visited (for blue coloring)
            ...
            for direction in Direction:
                ...
                # Outside board?
                if neighbor.check_bounds(GRID_WIDTH, GRID_HEIGHT):
                    continue

                # Can't go into the snake's body
                if neighbor in blocked:
                    continue

                # Movement cost
                step_cost = 1
                if neighbor in obstacle_positions:
        ...
        # If we never got to the goal, there is no path
    ...
    def turn(self, direction: Direction):
        # Not used; this player is fully automatic
        pass

if __name__ == "__main__":
    snake = Snake(WIDTH, WIDTH, INIT_LENGTH)
    # player = HumanPlayer()
    player = AStarPlayer()           # A*
