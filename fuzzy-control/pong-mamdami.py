...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        self.x_universe = np.arange(-W, W + 1, 1)
        self.y_universe = np.arange(0, H + 1, 1)
        self.v_universe = np.linspace(-1.0, 1.0, 201)

        self.x_dist = fuzzcontrol.Antecedent(self.x_universe, "x_diff")
        self.y_dist = fuzzcontrol.Antecedent(self.y_universe, "y_diff")
        self.velocity = fuzzcontrol.Consequent(self.v_universe, "velocity")

        self.x_dist["left"]  = fuzz.trapmf(self.x_universe, [-W, -W, -W*0.15, -W*0.02])
        self.x_dist["center"]= fuzz.trimf(self.x_universe,  [-W*0.03, 0, W*0.03])
        self.x_dist["right"] = fuzz.trapmf(self.x_universe, [ W*0.02,  W*0.15,  W,    W])

        self.y_dist["near"] = fuzz.trapmf(self.y_universe, [0, 0, H*0.12, H*0.28])
        self.y_dist["far"]  = fuzz.trapmf(self.y_universe, [H*0.22, H*0.5, H, H])

        self.velocity["fast_left"]  = fuzz.trapmf(self.v_universe, [-1.0, -1.0, -0.8, -0.3])
        self.velocity["fast_right"] = fuzz.trapmf(self.v_universe, [ 0.3,  0.8,  1.0,  1.0])

        rules = [
            fuzzcontrol.Rule(self.x_dist["left"]  & self.y_dist["near"], self.velocity["fast_right"]),
            fuzzcontrol.Rule(self.x_dist["left"]  & self.y_dist["far"],  self.velocity["fast_right"]),
            fuzzcontrol.Rule(self.x_dist["right"] & self.y_dist["near"], self.velocity["fast_left"]),
            fuzzcontrol.Rule(self.x_dist["right"] & self.y_dist["far"],  self.velocity["fast_left"]),
            fuzzcontrol.Rule(self.x_dist["center"] & self.y_dist["near"], self.velocity["fast_right"]),
            fuzzcontrol.Rule(self.x_dist["center"] & self.y_dist["far"],  self.velocity["fast_left"]),
        ]

        system = fuzzcontrol.ControlSystem(rules)
        self.ctrl = fuzzcontrol.ControlSystemSimulation(system)

        self._last_cmd = 1

    def act(self, x_diff: int, y_diff: int):
        x_diff = float(np.clip(x_diff, self.x_universe[0], self.x_universe[-1]))
        y_diff = float(np.clip(y_diff, self.y_universe[0], self.y_universe[-1]))

        self.ctrl.input["x_diff"] = x_diff
        self.ctrl.input["y_diff"] = y_diff
        self.ctrl.compute()
        v = float(self.ctrl.output["velocity"])

        deadband = max(1.0, self.board.surface.get_width() * 0.0025)
        if abs(x_diff) <= deadband:
            cmd = self._last_cmd
        else:
            cmd = 1 if v >= 0 else -1
        self._last_cmd = cmd
        ...
        far = 10**6
        target_x = self.racket.rect.x + (far if cmd > 0 else -far)
        self.move(target_x)
